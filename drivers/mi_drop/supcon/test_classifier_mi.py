...
def main_worker(gpu, n_gpus, args) :
    ...
    # add new classes to keep track of the new class ids
    test_dataset_new = \
        getattr(
            importlib.import_module(
                '.' + args.dataset, package='datasets'), 
    ...
    local_class_l = deepcopy(test_dataset_new.keep_class_l);
    test_dataset.merge_dataset(test_dataset_new);
...
def test_validate( 
    ...
    if args.gpu == 0 :
        print(f"Accuracy (Global) = {iou_meter.accuracies():.1f} %");
        print(f"Local class ids = {local_class_l}");
        print(f"Accuracy (Local) = {iou_meter.accuracies(local_class_l):.1f} %")
