...
import utils
from .lwf import LwF
from losses.helpers import one_hot


class LwF_MC(LwF):

    def __init__(self, cfg, cfg_data, args, is_train, is_distributed, n_gpus):
        super(LwF_MC, self).__init__(cfg, cfg_data, args, is_train, is_distributed, n_gpus)
    ...
    def train_epoch(self, tbar, epoch, train_logger, current_t_index) :
        ...
        while bi <= n_batches :
            ...
            for lname in self.criteria:
                target_mod = one_hot(target, self.valid_out_dim)
                lfunc = self.criteria[lname].func
                lweight = self.criteria[lname].weight
                if  output_hat is not None:
                    target_mod[:, :self.last_valid_out_dim] = torch.sigmoid(output_hat)
                    lval = lfunc(output, target_mod)
                else:
                    lval = lfunc(output, target_mod)
